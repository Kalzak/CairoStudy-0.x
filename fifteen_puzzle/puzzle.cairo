...
func main{output_ptr : felt*, range_check_ptr}():
  ...
  # Get the value of the frame pointer register (fp) so that we can use the address of loc0
...
func check_solution{output_ptr : felt*, range_check_ptr}(loc_list : Location*, tile_list : felt*, n_steps):
  ...
  # Store range_check_ptr in a local variable to make it accessible after the call to output_initial_values()
  local range_check_ptr = range_check_ptr

  # Verify that the squashed dict has exactly 15 entries
  # This will guarantee that all values in the tile list are in the range 1-15
...
func verify_adjacent_locations(loc0 : Location*, loc1 : Location*):
  ...
  if row_diff == 0:
    ...
    # Make sure the difference in col is 1 or -1
...
func verify_location_list(loc_list : Location*, n_steps):
  ...
  # (remember that there is always one more location than steps)
